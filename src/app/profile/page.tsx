...
function ProfileLink({ href, label, sub }: { href: string; label: string; sub: string }) {
  return (
    ...
    >
      <span>
        <span className="block text-sm font-semibold text-orbit-text">{label}</span>
  ...
  return (
    <AppShell>
      ...
      <div className="flex flex-col items-center gap-1.5 px-6 py-6">
        <span className="flex h-16 w-16 items-center justify-center rounded-full bg-orbit-card2 text-lg font-semibold text-orbit-text">
          {user.name.slice(0, 2).toUpperCase()}
        </span>
        <p className="mt-1 text-lg font-semibold text-orbit-text">{user.name}</p>

...
import { LOW_USAGE_THRESHOLD } from "@/lib/agents/shield";
...
function usageLabel(usageScore: number, lastUsedAt?: string): { text: string; tone: "green" | "amber" | "red" } {
  if (usageScore >= 0.7) return { text: "Used regularly", tone: "green" };
  if (usageScore >= LOW_USAGE_THRESHOLD) return { text: "Low usage", tone: "amber" };
  const since = lastUsedAt ? daysSince(lastUsedAt) : null;
...
export default async function ShieldPage() {
  ...
  const flaggedCount = activeSubs.filter((s) => s.usageScore < LOW_USAGE_THRESHOLD).length;
    ...
    <AppShell withBottomNav={false}>
      ...
      <div className="mx-4 mt-3 rounded-2xl border-l-4 border-orbit-shield bg-orbit-card p-4">
        ...
        <div className="flex">
          <div className="flex-1 border-r border-orbit-border pr-2">
            <p className="text-[22px] font-bold text-white">{formatSGD(monthlyTotal)}</p>
            <p className="mt-0.5 text-xs text-orbit-muted">monthly subscriptions</p>
          </div>
      ...
      <div className="mx-4 mt-3 flex-1 overflow-hidden rounded-2xl bg-orbit-card">
        ...
        {graph.subscriptions.map((sub) => {
          const usage = usageLabel(sub.usageScore, sub.lastUsedAt);
